...
public class SysMenuController extends AbstractController {
    ...
    public List<SysMenuEntity> list(){
        //第一种写法，借助mybatis-plus
        List<SysMenuEntity> list = sysMenuService.list();

        if(list != null && !list.isEmpty()){
            list.stream().forEach(sysMenuEntity -> {
                if(Constant.MenuType.BUTTON.getValue() == sysMenuEntity.getType()){
                    SysMenuEntity menu = sysMenuService.getById(sysMenuEntity.getParentId());
                    sysMenuEntity.setParentName((menu != null && StringUtils.isNotBlank(menu.getName())) ? menu.getName() : "");
                }
                /*SysMenuEntity menu = sysMenuService.getById(sysMenuEntity.getParentId());
                sysMenuEntity.setParentName((menu != null && StringUtils.isNotBlank(menu.getName())) ? menu.getName() : "");*/
            });
